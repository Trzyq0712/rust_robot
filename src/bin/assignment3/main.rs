...
static G_ADC1: Mutex<RefCell<Option<stm32::ADC1>>> = Mutex::new(RefCell::new(None));

static G_INFRARED: Mutex<Cell<(u16, u16)>> = Mutex::new(Cell::new((0, 0)));
...
fn main() -> ! {
    ...
    intr::free(|cs| {
        G_ADC1.borrow(cs).replace(Some(adc1));
    });
    ...
    loop {
        let (left, right) = intr::free(|cs| G_INFRARED.borrow(cs).get());
